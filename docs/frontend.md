...
現状は「タイトル画面 -> ソロゲーム画面 -> 正解演出」までが主フローで、結果オーバーレイ遷移は実装途中です。
...
- 演出完了後に `ResultOverlay` を表示する意図の実装が入っているが、現状は遷移が完了しない
...
4. `ThanksConfetti` に `onClose` を渡しているが、コンポーネント側で props を受けていない。
5. `ResultOverlay` の Escape キー処理は `onClose` を呼び出さない書き方になっている（参照のみ）。
6. APIエラー時は `console.error` のみで、ユーザー向けエラーUIは未実装。
7. `MessageBubble` のデフォルトアイコンが外部URL依存。
...
6. 結果オーバーレイは遷移ロジックが未完了のため、現状フローでは到達しない
